...
1. **Early filters** — `identified`, `filter`, `segment` (reduce rows before expensive ops)
1. **Transform** — `eval`, `select`, `set` (compute/rename fields)
1. **Group / Reduce** — Aggregate data per key or across all rows
1. **Enrich** — `bulkExpand`, `merge`, `expand` (add metadata AFTER reducing row count)
1. **Post-group operations** — Additional `group`, `filter`, `select` on aggregated data
1. **Sort + Limit** — Order and cap results
...
1. Any caveats (e.g., "this uses `daily` so you'll get one result per day")
1. Placeholders marked clearly: `<YOUR_APP_ID>`, `<YOUR_PAGE_ID>`, etc.
